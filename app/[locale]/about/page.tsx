      ...
      <section className="px-6 lg:px-10 py-24 lg:py-32 border-b border-[var(--stroke)]">
        <div className="mx-auto max-w-[1400px] grid-12 gap-y-10">
          ...
          <div className="col-span-12 lg:col-span-7 lg:col-start-6 space-y-8">
            <p className="font-serif text-h3 font-light leading-[1.3] tracking-tight text-[var(--text)]">
              {t("manifestoP1")}
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-[var(--stroke)]">
            {VALUES.map((value, i) => (
              ...
              >
                ...
                <h3 className="font-serif text-[2rem] lg:text-[2.5rem] font-light tracking-tight leading-none">
                  {value.title}
                </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
            {TEAM.map((member, i) => (
              <article key={i} className="group">
                <div className="aspect-[4/5] bg-[var(--surface-1)] border border-[var(--stroke)] mb-6 flex items-center justify-center font-serif text-[clamp(4rem,8vw,7rem)] font-light text-[var(--text-faint)] group-hover:text-[var(--accent)] transition-colors">
                  {member.initials}
                </div>
                <h3 className="font-serif text-[1.5rem] font-light tracking-tight">{member.name}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-[var(--stroke)] border border-[var(--stroke)]">
            {CITIES.map((city, i) => (
              <div key={i} className="p-8 lg:p-12 bg-[var(--bg)]">
                ...
                <div className="font-serif text-[clamp(2.5rem,5vw,4rem)] font-light tracking-tight leading-none mt-4">
                  {city.name}
                </div>
